...
#	try:
	...
	for row in rows:
		...
		if len(aka_list)==0:
			cassandra.execute("INSERT INTO tbl_actors_by_name (name,"+column+",movies_id,movies_title"+") values(\'"+name+"\',"+values+","+movies_id+","+movies_name+")")
		else:
			cassandra.execute("INSERT INTO tbl_actors_by_name (name,"+column+",aka,movies_id,movies_title"+") values(\'"+name+"\',"+values+","+str(aka_list_write)+","+movies_id+","+movies_name+")")
		#print query
