...
query =""
item_amount=100
for i in xrange(1263600,maxid+1,item_amount):
...
#	try:
	...
	for row in rows:
		...
		if not row[1] is None:
			for e in row[1].split():
				name_list.append(e.lower().replace("\'"," "))
		...
		if len(aka_list)==0:
			for e in name_list:
				cassandra.execute("INSERT INTO tbl_actors_by_name (name,"+column+",movies_id,movies_title"+") values(\'"+e+"\',"+values+","+movies_id+","+movies_name+")")
		else:
			for e in name_list:
				cassandra.execute("INSERT INTO tbl_actors_by_name (name,"+column+",aka,movies_id,movies_title"+") values(\'"+e+"\',"+values+","+str(aka_list_write)+","+movies_id+","+movies_name+")")
		#print query
